import {
  DebugServerError,
  scheduleChannelPerformanceLogging,
...
describe('scheduleChannelPerformanceLogging', () => {
  ...
  it('schedule channel stats logging', async () => {
    const interval = 10;
    const executor = {
      execute: sinon
        .mock()
    ...
    const result = await scheduleChannelPerformanceLogging(executor, {
      interval,
    });
    assert.deepStrictEqual(result, interval);
  });

  it('unschedule channel stats logging', async () => {
    const interval = 0;
    const executor = {
      execute: sinon
        .mock()
